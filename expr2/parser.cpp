            ...
            { //遍历产生式右部
                ...
                { //遍历产生式右部所有字符
                    ...
                    else
                    {
                        ...
                        { //将 ch 的 First 集合加入到 nonTerminalChar 的 First 集合中
                            if (temp == '@')
                            {
                                flag = true;
                            }
                ...
                { //遍历所有产生式右部
                    ...
                    { //遍历产生式右部字符
                        for (; i < rightSide.length(); i++)
                        {
                            ...
                            { //找到与 nonTerminalChar 相同的字符
                                if (i == rightSide.length() - 1)
                                { //判断找到的非终结符是否是右部最后一个字符
                                    for (char ch : FOLLOW[iter->first])
                                    { //把产生式左部非终结符的 Follow 加入到 nonTerminalChar 的 Follow 集合中
                        ...
                        { //遍历后续字符
                            ...
                            { //是非终结符
                                ...
                                { //将该非终结符的 First 集合加入到 nonTerminalChar 的 Follow 集合中（除了空串）
                                    if (ch != '@')
                                    {
                                        FOLLOW[nonTerminalChar].insert(ch);
                                    }
                                ...
                                { //如果该非终结符的 First 集合中不存在空串，则退出循环
                                    break;
                                }
                                else if (i == rightSide.length() - 1)
                                { //该非终结符的 First 集合中存在空串，且为右部最后一个字符
                                    for (char ch : FOLLOW[iter->first])
                                    {
                                        FOLLOW[nonTerminalChar].insert(
...
void is_left_recursive(char s, string ss)
{ // 判断非终结符S是否有左递归
    if (left_recursive)
        return;
    if (ss[0] == s)
...
bool has_union(set<char> s, set<char> ss)
{ // 判断两个集合是否有交集
    for (auto i : s)
    {
        for (auto j : ss)
...
void createAnalysisTable()
{
    ...
    { //遍历文法的每个产生式
        ...
        {                                                        //遍历文法产生式右部
            ...
            { //遍历产生式右部 First 集合
                if (first == '@')
                { //如果产生式右部的 First 集合存在空串
                    for (char follow : FOLLOW[iter->first])
                    { //对产生式左部的 Follow 中的每个终结符，把该产生式加入到表格中对应位置
                        string temp;
                ...
                if (analysisTable.find(temp) == analysisTable.end())
                { //表项为空，对 产生式右部的 First 集合中的每个终结符，插入产生式
                    string temp2;
                    temp2.push_back(iter->first);
                    analysisTable[temp].push_back(temp2);
                    for (char ch : rightSide)
    ...
    for (char nonTerminalChar : nonTerminalSymbol)
    { //插入同步记号，通过 nonTerminalChar 的 FOLLOW 集合
...
void analysis()
{
    TablePrinter tp(&std::cout);
    tp.set_flush_left();
    tp.AddColumn("Stack", 20);
    tp.AddColumn("Input", 20);
    tp.AddColumn("Action", 30);
    tp.PrintHeader();
    ...
    { //当出现栈空，但输入串不为空的情况，需要跳过输入串中的字符，直到输入串空
        tp << "#";
        string temp;
